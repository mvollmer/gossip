...
static void gossip_canvas_line_class_init (GossipCanvasLineClass *class);
static void gossip_canvas_line_init       (GossipCanvasLine      *line);
static void gossip_canvas_line_destroy    (GtkObject            *object);
static void gossip_canvas_line_set_arg    (GtkObject            *object,
					  GtkArg               *arg,
					  guint                 arg_id);
static void gossip_canvas_line_get_arg    (GtkObject            *object,
					  GtkArg               *arg,
					  guint                 arg_id);

static void   gossip_canvas_line_set_coords  (GossipCanvasItem *item, int n_coords, double *coords);
static void   gossip_canvas_line_update      (GossipCanvasItem *item, double *affine, int flags);
static void   gossip_canvas_line_realize     (GossipCanvasItem *item);
static void   gossip_canvas_line_unrealize   (GossipCanvasItem *item);
static void   gossip_canvas_line_draw        (GossipCanvasItem *item, GdkDrawable *drawable,
					     int x, int y, int width, int height);
static double gossip_canvas_line_point       (GossipCanvasItem *item, double x, double y,
					     int cx, int cy, GossipCanvasItem **actual_item);
static void   gossip_canvas_line_translate   (GossipCanvasItem *item, double dx, double dy);
static void   gossip_canvas_line_bounds      (GossipCanvasItem *item, double *x1, double *y1, double *x2, double *y2);
#if 0
static void   gossip_canvas_line_render      (GossipCanvasItem *item, GossipCanvasBuf *buf);
#endif
...
GtkType
gossip_canvas_line_get_type (void)
{
	...
	if (!line_type) {
		GtkTypeInfo line_info = {
			"GossipCanvasLine",
			sizeof (GossipCanvasLine),
			sizeof (GossipCanvasLineClass),
			(GtkClassInitFunc) gossip_canvas_line_class_init,
			(GtkObjectInitFunc) gossip_canvas_line_init,
			NULL, /* reserved_1 */
			NULL, /* reserved_2 */
			(GtkClassInitFunc) NULL
		};

		line_type = gtk_type_unique (gossip_canvas_item_get_type (), &line_info);
	}

	return line_type;
}

#define GTK_TYPE_GOSSIP_CANVAS_POINTS GTK_TYPE_POINTER

static void
gossip_canvas_line_class_init (GossipCanvasLineClass *class)
{
...
#if 0
	gtk_object_add_arg_type ("GossipCanvasLine::points", GTK_TYPE_GOSSIP_CANVAS_POINTS, GTK_ARG_READWRITE, ARG_POINTS);
#endif
	gtk_object_add_arg_type ("GossipCanvasLine::fill_color", GTK_TYPE_STRING, GTK_ARG_WRITABLE, ARG_FILL_COLOR);
	gtk_object_add_arg_type ("GossipCanvasLine::fill_color_gdk", GTK_TYPE_GDK_COLOR, GTK_ARG_READWRITE, ARG_FILL_COLOR_GDK);
	gtk_object_add_arg_type ("GossipCanvasLine::fill_color_rgba", GTK_TYPE_UINT, GTK_ARG_READWRITE, ARG_FILL_COLOR_RGBA);
	gtk_object_add_arg_type ("GossipCanvasLine::fill_stipple", GTK_TYPE_GDK_WINDOW, GTK_ARG_READWRITE, ARG_FILL_STIPPLE);
	gtk_object_add_arg_type ("GossipCanvasLine::width_pixels", GTK_TYPE_UINT, GTK_ARG_WRITABLE, ARG_WIDTH_PIXELS);
	gtk_object_add_arg_type ("GossipCanvasLine::width_units", GTK_TYPE_DOUBLE, GTK_ARG_WRITABLE, ARG_WIDTH_UNITS);
	gtk_object_add_arg_type ("GossipCanvasLine::hit_width", GTK_TYPE_DOUBLE, GTK_ARG_WRITABLE, ARG_HIT_WIDTH);
	gtk_object_add_arg_type ("GossipCanvasLine::cap_style", GTK_TYPE_GDK_CAP_STYLE, GTK_ARG_READWRITE, ARG_CAP_STYLE);
	gtk_object_add_arg_type ("GossipCanvasLine::join_style", GTK_TYPE_GDK_JOIN_STYLE, GTK_ARG_READWRITE, ARG_JOIN_STYLE);
	gtk_object_add_arg_type ("GossipCanvasLine::line_style", GTK_TYPE_GDK_LINE_STYLE, GTK_ARG_READWRITE, ARG_LINE_STYLE);
	gtk_object_add_arg_type ("GossipCanvasLine::first_arrowhead", GTK_TYPE_BOOL, GTK_ARG_READWRITE, ARG_FIRST_ARROWHEAD);
	gtk_object_add_arg_type ("GossipCanvasLine::last_arrowhead", GTK_TYPE_BOOL, GTK_ARG_READWRITE, ARG_LAST_ARROWHEAD);
	gtk_object_add_arg_type ("GossipCanvasLine::smooth", GTK_TYPE_BOOL, GTK_ARG_READWRITE, ARG_SMOOTH);
	gtk_object_add_arg_type ("GossipCanvasLine::spline_steps", GTK_TYPE_UINT, GTK_ARG_READWRITE, ARG_SPLINE_STEPS);
	gtk_object_add_arg_type ("GossipCanvasLine::arrow_shape_a", GTK_TYPE_DOUBLE, GTK_ARG_READWRITE, ARG_ARROW_SHAPE_A);
	gtk_object_add_arg_type ("GossipCanvasLine::arrow_shape_b", GTK_TYPE_DOUBLE, GTK_ARG_READWRITE, ARG_ARROW_SHAPE_B);
	gtk_object_add_arg_type ("GossipCanvasLine::arrow_shape_c", GTK_TYPE_DOUBLE, GTK_ARG_READWRITE, ARG_ARROW_SHAPE_C);

	object_class->destroy = gossip_canvas_line_destroy;
	object_class->set_arg = gossip_canvas_line_set_arg;
	object_class->get_arg = gossip_canvas_line_get_arg;

	item_class->set_coords = gossip_canvas_line_set_coords;
	item_class->update = gossip_canvas_line_update;
	item_class->realize = gossip_canvas_line_realize;
	item_class->unrealize = gossip_canvas_line_unrealize;
	item_class->draw = gossip_canvas_line_draw;
	item_class->point = gossip_canvas_line_point;
	item_class->translate = gossip_canvas_line_translate;
	item_class->bounds = gossip_canvas_line_bounds;

#if 0
	item_class->render = gossip_canvas_line_render;
#endif
}

static void
gossip_canvas_line_init (GossipCanvasLine *line)
{
	line->width = 0.0;
	line->hit_width = 0.0;
...
static void
gossip_canvas_line_destroy (GtkObject *object)
{
	GossipCanvasLine *line;

	g_return_if_fail (object != NULL);
	g_return_if_fail (GOSSIP_IS_CANVAS_LINE (object));

	line = GOSSIP_CANVAS_LINE (object);
 ...
 */
static void
get_bounds (GossipCanvasLine *line, double *bx1, double *by1, double *bx2, double *by2)
{
	double *coords;
	double x1, y1, x2, y2;
	double width;
	...
	if (line->width_pixels)
		width = (line->width? line->width:1) / line->item.canvas->pixels_per_unit;
	else
		width = line->width;
	...
	if (line->join == GDK_JOIN_MITER)
		for (i = line->num_points, coords = line->coords; i >= 3; i--, coords += 2) {
			...
			if (gossip_canvas_get_miter_points (coords[0], coords[1],
							   coords[2], coords[3],
							   coords[4], coords[5],
							   width,
							   &mx1, &my1, &mx2, &my2)) {
...
/* Computes the bounding box of the two segments adjacent to a point,
 * including arrow points.  This is the same routine as get_bounds,
 * but only the points with index point-1, point, and point+1 are
 * considered (if they exist). */

static void
get_point_influence_bounds (GossipCanvasLine *line, int point, double *bx1, double *by1, double *bx2, double *by2)
{
  double *coords;
  double x1, y1, x2, y2;
  double width;
  ...
  x1 = x2 = line->coords[index];
  y1 = y2 = line->coords[index+1];
  if (point > 0)
    GROW_BOUNDS (x1, y1, x2, y2, line->coords[index-2], line->coords[index-1]);
  if (point < line->num_points-1)
    GROW_BOUNDS (x1, y1, x2, y2, line->coords[index+2], line->coords[index+3]);
  ...
  if (line->join == GDK_JOIN_MITER)
    for (i = point-1, coords = line->coords+index-2; i <= point+1; i++, coords += 2)
      {
	double mx1, my1, mx2, my2;
	if (i > 0 && i < line->num_points-1)
	  {
	    if (gossip_canvas_get_miter_points (coords[-2], coords[-1],
						coords[0], coords[1],
						coords[2], coords[3],
						width,
						&mx1, &my1, &mx2, &my2))
  ...
  if (point == 0 && line->first_arrow && line->first_coords)
    for (i = 0, coords = line->first_coords; i < NUM_ARROW_POINTS; i++, coords += 2)
      GROW_BOUNDS (x1, y1, x2, y2, coords[0], coords[1]);
  
  if (point == line->num_points-1 && line->last_arrow && line->last_coords)
    for (i = 0, coords = line->last_coords; i < NUM_ARROW_POINTS; i++, coords += 2)
      GROW_BOUNDS (x1, y1, x2, y2, coords[0], coords[1]);
 ...
 */
static void
get_bounds_canvas (GossipCanvasLine *line, double *bx1, double *by1, double *bx2, double *by2, double affine[6])
{
...
static void
get_point_influence_bounds_canvas (GossipCanvasLine *line, int point, int *cx1, int *cy1, int *cx2, int *cy2)
{
  GossipCanvasItem *item = GOSSIP_CANVAS_ITEM(line);
  double x1, y1, x2, y2;
  double dx, dy;

  get_point_influence_bounds (line, point, &x1, &y1, &x2, &y2);
...
static void
recalc_bounds (GossipCanvasLine *line)
{
	GossipCanvasItem *item;
	double x1, y1, x2, y2;
	int cx1, cx2, cy1, cy2;
...
static void
reconfigure_arrows (GossipCanvasLine *line)
{
	double *poly, *coords;
	double dx, dy, length;
...
static void
set_line_gc_foreground (GossipCanvasLine *line)
{
	GdkColor c;
...
static void
set_line_gc_width (GossipCanvasLine *line)
{
	int width;
...
static void
set_stipple (GossipCanvasLine *line, GdkBitmap *stipple, int reconfigure)
{
...
static void
reconfigure_arrows_and_bounds (GossipCanvasLine *line)
{
...
static void
gossip_canvas_line_set_coords (GossipCanvasItem *item, int n_coords, double *coords)
{
  GossipCanvasLine *line = GOSSIP_CANVAS_LINE(item);
  ...
  line->num_points = n_coords/2;
  line->coords = g_new (double, 2 * line->num_points);
  memcpy (line->coords, coords, 2 * line->num_points * sizeof (double));
...
static void
gossip_canvas_line_set_arg (GtkObject *object, GtkArg *arg, guint arg_id)
{
	GossipCanvasItem *item;
	GossipCanvasLine *line;
	...
	case ARG_FILL_COLOR:
		gossip_canvas_get_color (item->canvas, GTK_VALUE_STRING (*arg), &color);
	...
	case ARG_FILL_COLOR_RGBA:
		...
		gossip_canvas_item_request_redraw_svp (item, line->fill_svp);
		break;
#endif
	...
	case ARG_HIT_WIDTH:
		line->hit_width = fabs (GTK_VALUE_DOUBLE (*arg));
		break;
...
static void
gossip_canvas_line_get_arg (GtkObject *object, GtkArg *arg, guint arg_id)
{
	GossipCanvasLine *line;
	GossipCanvasPoints *points;
	GdkColor *color;
	...
	case ARG_POINTS:
		if (line->num_points != 0) {
			points = gossip_canvas_points_new (line->num_points);
...
void
gossip_canvas_line_move_point (GossipCanvasLine *line, int index, double dx, double dy)
{
  GossipCanvasItem *item = GOSSIP_CANVAS_ITEM(line);
  int x1, y1, x2, y2;
  ...
  if (index < 0 || index >= line->num_points)
    return;

  /* If the item is visible, requests a redraw of it. */
  if (item->object.flags & GOSSIP_CANVAS_ITEM_VISIBLE)
    {
      get_point_influence_bounds_canvas (line, index, &x1, &y1, &x2, &y2);
      gossip_canvas_request_redraw (item->canvas, x1, y1, x2, y2);
    }

  line->coords[2*index]   += dx;
  line->coords[2*index+1] += dy;
  /* XXX - do we need to do the arrowhead blues? */
  recalc_bounds (line);
...
#if 0
static void
gossip_canvas_line_render (GossipCanvasItem *item,
			     GossipCanvasBuf *buf)
{
	GossipCanvasLine *line;
	guint32 fg_color, bg_color;

	line = GOSSIP_CANVAS_LINE (item);

	if (line->fill_svp != NULL)
		gossip_canvas_render_svp (buf, line->fill_svp, line->fill_rgba);

	if (line->first_svp != NULL)
		gossip_canvas_render_svp (buf, line->first_svp, line->fill_rgba);

	if (line->last_svp != NULL)
		gossip_canvas_render_svp (buf, line->last_svp, line->fill_rgba);
}
#endif
...
static void
gossip_canvas_line_update (GossipCanvasItem *item, double *affine, int flags)
{
	GossipCanvasLine *line;
	int i;
#if 0
	...
	if (item->canvas->aa) {
		...
		gossip_canvas_item_update_svp_clip (item, &line->fill_svp, svp, clip_path);
		...
		gossip_canvas_item_update_svp_clip (item, &line->first_svp, svp, clip_path);
		...
		gossip_canvas_item_update_svp_clip (item, &line->last_svp, svp, clip_path);
...
static void
gossip_canvas_line_realize (GossipCanvasItem *item)
{
	GossipCanvasLine *line;
...
static void
gossip_canvas_line_unrealize (GossipCanvasItem *item)
{
	GossipCanvasLine *line;
...
static void
gossip_canvas_line_draw (GossipCanvasItem *item, GdkDrawable *drawable,
			int x, int y, int width, int height)
{
	GossipCanvasLine *line;
	...
	if (line->stipple)
		gossip_canvas_set_stipple_origin (item->canvas, line->gc);
...
static double
gossip_canvas_line_point (GossipCanvasItem *item, double x, double y,
			 int cx, int cy, GossipCanvasItem **actual_item)
{
	GossipCanvasLine *line;
...
#ifdef VERBOSE
	g_print ("gossip_canvas_line_point x, y = (%g, %g); cx, cy = (%d, %d)\n", x, y, cx, cy);
#endif
	...
	*actual_item = item;
	...
	for (i = num_points, coords = line_points; i >= 2; i--, coords += 2) {
		...
		if (i == num_points)
			gossip_canvas_get_butt_points (coords[2], coords[3], coords[0], coords[1],
			...
			if ((line->join == GDK_JOIN_BEVEL) || changed_miter_to_bevel) {
				...
				dist = gossip_canvas_polygon_to_point (poly, 5, x, y);
				if (dist < GOSSIP_CANVAS_EPSILON) {
					best = 0.0;
					goto done;
				} else if (dist < best)
		...
		if (i == 2)
			gossip_canvas_get_butt_points (coords[0], coords[1], coords[2], coords[3],
						      width, (line->cap == GDK_CAP_PROJECTING),
	...
	if (line->first_arrow) {
		dist = gossip_canvas_polygon_to_point (line->first_coords, NUM_ARROW_POINTS, x, y);
		if (dist < GOSSIP_CANVAS_EPSILON) {
			best = 0.0;
			goto done;
		} else
	...
	if (line->last_arrow) {
		dist = gossip_canvas_polygon_to_point (line->last_coords, NUM_ARROW_POINTS, x, y);
		if (dist < GOSSIP_CANVAS_EPSILON) {
			best = 0.0;
			goto done;
		} else
...
static void
gossip_canvas_line_translate (GossipCanvasItem *item, double dx, double dy)
{
	GossipCanvasLine *line;
	int i;
	double *coords;
...
static void
gossip_canvas_line_bounds (GossipCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	GossipCanvasLine *line;
